...
fn manage_backup_space(backup_root: &str, max_space: u64) -> io::Result<()> {
    ...
    // Collect information about existing backups
    if path.exists() {
...
mod tests {
    ...
    use super::*;
    use crate::test_helpers::test_helpers::{create_tmp_folder, file_contents_matches, time_fixer};
    ...
    fn test_backup_with_options() -> io::Result<()> {
        ...
        // Verify backup
        assert_eq!(file_contents_matches(
...
fn test_manage_backup_space() -> io::Result<()> {
    ...
    let remaining_backups: Vec<_> = fs::read_dir(&backup_root)?
        ...
        .collect();
    let remaining_size: u64 = remaining_backups.iter().map(|entry| {
        entry
            .path()
            .join("large_file.txt")
            .metadata()
